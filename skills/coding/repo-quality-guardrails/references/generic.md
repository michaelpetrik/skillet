...
- the most established offline-capable documentation and visualization tooling the active language or framework offers, pinned in repo-owned manifests rather than chosen ad hoc per developer machine.
...
- a repo-local docs gate that generates human-readable docs and at least one architecture artifact offline with pinned tools;
...
- documentation generation and rendered architecture artifacts from established local tooling;
...
- If the ecosystem has no credible local documentation generator or visualizer, record the research and mark that documentation control `blocker` until a defendable fallback is selected.
